...
# recurrence KM by GARD quintile by site

```{r plot_recur}

gard_quint_rec <- rt.rec %>%
  group_by(Site) %>%
  summarise(top_cutoff = quantile(GARD, 0.90),
            bottom_cutoff = quantile(GARD, 0.10))

breast_high <- as.numeric(gard_quint_rec[gard_quint_rec$Site=="breast", "top_cutoff"])
breast_low <-  as.numeric(gard_quint_rec[gard_quint_rec$Site=="breast", "bottom_cutoff"])
breast_TN_high <- as.numeric(gard_quint_rec[gard_quint_rec$Site=="breast_TN", "top_cutoff"])
breast_TN_low <-  as.numeric(gard_quint_rec[gard_quint_rec$Site=="breast_TN", "bottom_cutoff"])
endometrial_high <- as.numeric(gard_quint_rec[gard_quint_rec$Site=="endometrial", "top_cutoff"])
endometrial_low <-  as.numeric(gard_quint_rec[gard_quint_rec$Site=="endometrial", "bottom_cutoff"])
HN_high <- as.numeric(gard_quint_rec[gard_quint_rec$Site=="HN", "top_cutoff"])
HN_low <-  as.numeric(gard_quint_rec[gard_quint_rec$Site=="HN", "bottom_cutoff"])
lung_high <- as.numeric(gard_quint_rec[gard_quint_rec$Site=="lung", "top_cutoff"])
lung_low <-  as.numeric(gard_quint_rec[gard_quint_rec$Site=="lung", "bottom_cutoff"])
melanoma_high <- as.numeric(gard_quint_rec[gard_quint_rec$Site=="melanoma", "top_cutoff"])
melanoma_low <-  as.numeric(gard_quint_rec[gard_quint_rec$Site=="melanoma", "bottom_cutoff"])


km_data_rec_grp <- rt.rec %>%
  mutate(GARD_class = case_when(Site == "breast" & GARD > breast_high ~ "GARD_High",
                                Site == "breast_TN" & GARD > breast_TN_high ~ "GARD_High",
                                Site == "endometrial" & GARD > endometrial_high ~ "GARD_High",
                                Site == "HN" & GARD > HN_high ~ "GARD_High",
                                Site == "lung" & GARD > lung_high ~ "GARD_High",
                                Site == "melanoma" & GARD > melanoma_high ~ "GARD_High",
                                Site == "breast" & GARD < breast_low ~ "GARD_Low",
                                Site == "breast_TN" & GARD < breast_TN_low ~ "GARD_Low",
                                Site == "endometrial" & GARD < endometrial_low ~ "GARD_Low",
                                Site == "HN" & GARD < HN_low ~ "GARD_Low",
                                Site == "lung" & GARD < lung_low ~ "GARD_Low",
                                Site == "melanoma" & GARD < melanoma_low ~ "GARD_Low")) %>%
  drop_na(GARD_class)
...
ggsurvplot(surv_fit, data = km_data_rec_grp, size = 1,
           ...
           title = "KM: Recurrence between Cohorts separated by GARD")
...
breast_TN_high <- as.numeric(gard_quint_os[gard_quint_os$Site=="breast_TN", "top_cutoff"])
breast_TN_low <-  as.numeric(gard_quint_os[gard_quint_os$Site=="breast_TN", "bottom_cutoff"])
endometrial_high <- as.numeric(gard_quint_os[gard_quint_os$Site=="endometrial", "top_cutoff"])
endometrial_low <-  as.numeric(gard_quint_os[gard_quint_os$Site=="endometrial", "bottom_cutoff"])
glioma_high <- as.numeric(gard_quint_os[gard_quint_os$Site=="glioma", "top_cutoff"])
glioma_low <-  as.numeric(gard_quint_os[gard_quint_os$Site=="glioma", "bottom_cutoff"])
lung_high <- as.numeric(gard_quint_os[gard_quint_os$Site=="lung", "top_cutoff"])
lung_low <-  as.numeric(gard_quint_os[gard_quint_os$Site=="lung", "bottom_cutoff"])
melanoma_high <- as.numeric(gard_quint_os[gard_quint_os$Site=="melanoma", "top_cutoff"])
melanoma_low <-  as.numeric(gard_quint_os[gard_quint_os$Site=="melanoma", "bottom_cutoff"])
pancreas_high <- as.numeric(gard_quint_os[gard_quint_os$Site=="pancreas", "top_cutoff"])
pancreas_low <-  as.numeric(gard_quint_os[gard_quint_os$Site=="pancreas", "bottom_cutoff"])


km_data_os_grp <- rt.os %>%
  mutate(GARD_class = case_when(Site == "glioma" & GARD > glioma_high ~ "GARD_High",
                                Site == "breast_TN" & GARD > breast_TN_high ~ "GARD_High",
                                Site == "endometrial" & GARD > endometrial_high ~ "GARD_High",
                                Site == "pancreas" & GARD > pancreas_high ~ "GARD_High",
                                Site == "lung" & GARD > lung_high ~ "GARD_High",
                                Site == "melanoma" & GARD > melanoma_high ~ "GARD_High",
                                Site == "glioma" & GARD < glioma_low ~ "GARD_Low",
                                Site == "breast_TN" & GARD < breast_TN_low ~ "GARD_Low",
                                Site == "endometrial" & GARD < endometrial_low ~ "GARD_Low",
                                Site == "pancreas" & GARD < pancreas_low ~ "GARD_Low",
                                Site == "lung" & GARD < lung_low ~ "GARD_Low",
                                Site == "melanoma" & GARD < melanoma_low ~ "GARD_Low")) %>%
  drop_na(GARD_class)

km_data_os_grp <- rt.os %>%
  mutate(GARD_class = case_when(Site == "glioma" & GARD > 21 ~ "GARD_High",
                                Site == "breast_TN" & GARD > 21 ~ "GARD_High",
                                Site == "endometrial" & GARD > 21 ~ "GARD_High",
                                Site == "pancreas" & GARD > 21 ~ "GARD_High",
                                Site == "lung" & GARD > 21 ~ "GARD_High",
                                Site == "melanoma" & GARD > 21 ~ "GARD_High",
                                Site == "glioma" & GARD < 21 ~ "GARD_Low",
                                Site == "breast_TN" & GARD < 21 ~ "GARD_Low",
                                Site == "endometrial" & GARD < 21 ~ "GARD_Low",
                                Site == "pancreas" & GARD < 21 ~ "GARD_Low",
                                Site == "lung" & GARD < 21 ~ "GARD_Low",
                                Site == "melanoma" & GARD < 21 ~ "GARD_Low")) %>%
  drop_na(GARD_class)
...
ggsurvplot(surv_fit, data = km_data_os_grp, size = 1,
           ...
           # break.time.by = 12, # break X axis in time intervals
           ggtheme = theme_bw(), # Change ggplot2 theme
           title = "KM: Overall Survival between Cohorts separated by GARD")



```

# recurrence KM by GARD quintile by cohort
...
km_data_rec_grp <- rt.rec %>%
  mutate(GARD_class = case_when(Cohort == "Breast (Erasmus)" & GARD > 21 ~ "GARD_High",
                                Cohort == "Breast (Erasmus)" & GARD < 21 ~ "GARD_Low",
                                Cohort == "Breast (Karolinksa)" & GARD > 21 ~ "GARD_High",
                                Cohort == "Breast (Karolinksa)" & GARD < 21 ~ "GARD_Low",
                                Cohort == "Breast (NKI)" & GARD > 21 ~ "GARD_High",
                                Cohort == "Breast (NKI)" & GARD < 21 ~ "GARD_Low",
                                Cohort == "Endometrium (TCC)" & GARD > 21 ~ "GARD_High",
                                Cohort == "Endometrium (TCC)" & GARD < 21 ~ "GARD_Low",
                                Cohort == "Head & Neck (NKI)" & GARD > 21 ~ "GARD_High",
                                Cohort == "Head & Neck (NKI)" & GARD < 21 ~ "GARD_Low",
                                ...
                                Cohort == "TNBC (NKI)" & GARD > 21 ~ "GARD_High",
                                Cohort == "TNBC (NKI)" & GARD < 21 ~ "GARD_Low")) %>%
  drop_na(GARD_class)
...
ggsurvplot(surv_fit, data = km_data_rec_grp, size = 1,
           ...
           # break.time.by = 12, # break X axis in time intervals
           ggtheme = theme_bw(), # Change ggplot2 theme
           title = "KM: Recurrence between cohorts separated by GARD quintile")


```
# OS KM by GARD quintile by cohort
...
ggsurvplot(surv_fit, data = km_data_os_grp, size = 1,
           ...
           # break.time.by = 12, # break X axis in time intervals
           ggtheme = theme_bw(), # Change ggplot2 theme
           title = "KM: Overall Survival between cohorts separated by GARD quintile")
...
### make head and neck regression line 
data_hn <- data %>%
  filter(Site=="Head & Neck")

lm_results_hn <- lm(GARD ~ 0 + RT_Dose , data = data_hn)

xs = c(35, 80)
beta = c(0, lm_results_hn$coefficients)
ys = cbind(1, xs) %*% beta

lm_fitted_hn <- data.frame(predict(lm_results_hn, newdata = data.frame(RT_Dose = seq(xs[1], xs[2], by = 1)), interval = 'confidence'))
lm_fitted_hn_df <- data.frame(RT_Dose = seq(xs[1], xs[2], by = 1), GARD_pred = lm_fitted_hn$fit, GARD_lower = lm_fitted_hn$lwr, GARD_upper = lm_fitted_hn$upr)
###

### find slope of all regression lines

for(subtype in unique(data$Site)){
  lm_site <- lm(GARD ~ 0 + RT_Dose, data = data %>% filter(Site==subtype))
  print(subtype)
  print(lm_site$coefficients)
}
...
dot_plot<-ggplot(data) + 
  geom_point(aes(x=RT_Dose, y=GARD, color = Site), size=1,
             position=position_jitterdodge(jitter.width = 1.2, jitter.height = 0)) +
  geom_smooth(aes(x=RT_Dose, y=GARD, group = Site, color = Site),
              method='lm', fullrange=TRUE, formula=y~x-1, se = TRUE,
              alpha = 0.22) +
  ## H&N curve + CI
  geom_segment(aes(x = xs[1], xend = xs[2], y = ys[1], yend = ys[2]), color = "#ff793f", size = 1) +
  geom_ribbon(data = lm_fitted_hn_df,
              aes(x = RT_Dose, y = GARD_pred,
                  ymin = GARD_lower,
                  ymax = GARD_upper),
              alpha = 0.15) +
  ##
...
hist_left <- ggplot(filter(kde.df2,Value=='GARD')) +
  geom_line(aes(x=y,y=x,color=Site),size=0.5,orientation = "y") +
  geom_area(aes(x=y,y=x,fill=Site), orientation = "y",alpha=0.6) +
  scale_color_manual(values=q2[Limits]) +
  scale_fill_manual(values=q2[Limits]) +
...
gg_joint <- grid.arrange(grobs=grobs,
             layout_matrix = cbind(c(1,2),c(4,3)),
             widths=c(4,1),heights=c(1,4))

grobs <- list(hist_top,dot_plot,hist_left,leg)

gg_joint <- grid.arrange(grobs=grobs,
             layout_matrix = cbind(c(4, 3),c(1,2)),
             widths=c(1,4),heights=c(1,4))

ggsave(here("Plots", "joint_plot.png"), gg_joint)
